...
{
    // true because making a memory reset pending in the update cycle causes resets after reconnects.
    // Instead, just call Reset from a single thread
    FreeMemory(true);
    if (resetVariables)
    {
        InitializeVariables();
...
void ReliabilityLayer::FreeThreadSafeMemory(void)
{
    ...
    {
        for (unsigned j = 0; j < splitPacketChannelList[i]->splitPacketList.Size(); j++)
        {
    ...
    {
        InternalPacket *internalPacket = outputQueue.Pop();
    ...
    */
    ...
    {
        for (unsigned j = 0; j < orderingHeaps[i].Size(); j++)
        {
...
#pragma warning( disable : 4127 ) // warning C4127: conditional expression is constant
    ...
    for (unsigned j = 0; j < outgoingPacketBuffer.Size(); j++)
    {
    ...
    if (dhf.isACK)
    {
        ...
        for (unsigned i = 0; i < incomingAcks.ranges.Size(); i++)
        {
                 ...
                 datagramNumber++)
            {
                ...
                {
                    for (unsigned int k = 0; k < unreliableWithAckReceiptHistory.Size();)
                    {
                        if (unreliableWithAckReceiptHistory[k].datagramNumber == datagramNumber)
                        {
                            ...
                            memcpy(ackReceipt->data + sizeof(MessageID),
                                   &unreliableWithAckReceiptHistory[k].sendReceiptSerial, sizeof(uint32_t));
...
//                 }
    ...
    else if (dhf.isNAK)
    {
        ...
        for (unsigned i = 0; i < incomingNAKs.ranges.Size(); i++)
        {
    ...
    else
    {
        ...
        for (uint32_t skippedMessageOffset = skippedMessageCount; skippedMessageOffset > 0; skippedMessageOffset--)
        ...
        while (internalPacket)
        {
                ...
                internalPacket->reliability == RELIABLE_ORDERED)
            {
                if (internalPacket->orderingChannel >= NUMBER_OF_ORDERED_STREAMS)
                {
                    ...
                    bpsMetrics[(int) USER_MESSAGE_BYTES_RECEIVED_IGNORED].Push1(timeRead, BITS_TO_BYTES(
                ...
                internalPacket->reliability == RELIABLE_ORDERED)
            {
                ...
                else if (holeCount > typeRange / (DatagramSequenceNumberType) 2)
                {
                    ...
                    for (unsigned int messageHandlerIndex = 0;
                         messageHandlerIndex < messageHandlerList.Size(); messageHandlerIndex++)
                        messageHandlerList[messageHandlerIndex]->OnReliabilityLayerNotification(
                                "holeCount > typeRange/(DatagramSequenceNumberType) 2", BYTES_TO_BITS(length),
                                systemAddress, false);
                ...
                internalPacket->reliability == RELIABLE_ORDERED)
            {
                ...
                if (internalPacket->orderingIndex == orderedReadIndex[internalPacket->orderingChannel])
                {
                    ...
                    else
                    {
                        ...
                        bpsMetrics[(int) USER_MESSAGE_BYTES_RECEIVED_PROCESSED].Push1(timeRead, BITS_TO_BYTES(
                                internalPacket->dataBitLength));
                        outputQueue.Push(internalPacket, _FILE_AND_LINE_);
...
//-------------------------------------------------------------------------------------------------------
bool
ReliabilityLayer::Send(char *data, BitSize_t numberOfBitsToSend, PacketPriority priority, PacketReliability reliability,
...
//        internalPacket->reliability == UNRELIABLE_SEQUENCED_WITH_ACK_RECEIPT
    ...
    RakAssert(internalPacket->dataBitLength < BYTES_TO_BITS(MAXIMUM_MTU_SIZE));
    RakAssert(!internalPacket->messageNumberAssigned);
    ...
    statistics.bytesInSendBuffer[(int) internalPacket->priority] += (double) BITS_TO_BYTES(
            internalPacket->dataBitLength);
...
#pragma warning( disable : 4127 ) // warning C4127: conditional expression is constant
    ...
    const bool hasDataToSendOrResend = !IsResendQueueEmpty() || bandwidthExceededStatistic;
    ...
    {
        for (unsigned int i = 0; i < RNS_PER_SECOND_METRICS_COUNT; i++)
    ...
    if (unreliableWithAckReceiptHistory.Size() > 0)
    {
        for (unsigned int i = 0; i < unreliableWithAckReceiptHistory.Size();)
        {
    ...
    if (hasDataToSendOrResend != 0)
    {
        ...
        if (retransmissionBandwidth > 0 || transmissionBandwidth > 0)
        {
            ...
            while ((int) BITS_TO_BYTES(allDatagramSizesSoFar) < retransmissionBandwidth)
            {
                ...
                while (!IsResendQueueEmpty())
                {
                    ...
                    {
                        BitSize_t nextPacketBitLength = internalPacket->headerLength + internalPacket->dataBitLength;
                        ...
                        internalPacket->retransmissionTime = congestionManager.GetRTOForRetransmission(
                                internalPacket->timesSent);
        ...
        if ((int) BITS_TO_BYTES(allDatagramSizesSoFar) < transmissionBandwidth)
        {
                     ...
                     (countdownToNextPacketPair == 0 && datagramsToSendThisUpdateIsPair.Size() == 1)))
            {
                ...
                statistics.isLimitedByOutgoingBandwidthLimit = bitsPerSecondLimit != 0 &&
                                                               BITS_TO_BYTES(bitsPerSecondLimit) <
                                                               bpsMetrics[USER_MESSAGE_BYTES_SENT].GetBPS1(time);


                while (outgoingPacketBuffer.Size() && !statistics.isLimitedByOutgoingBandwidthLimit)
                    //while ( sendPacketSet[ i ].Size() )
                {
                    InternalPacket *internalPacket = outgoingPacketBuffer.Peek();
                    RakAssert(!internalPacket->messageNumberAssigned);
                    ...
                    bool isReliable = internalPacket->reliability == RELIABLE ||
                                      internalPacket->reliability == RELIABLE_SEQUENCED ||
                                      internalPacket->reliability == RELIABLE_ORDERED ||
                                      internalPacket->reliability == RELIABLE_WITH_ACK_RECEIPT ||
                                      // internalPacket->reliability == RELIABLE_SEQUENCED_WITH_ACK_RECEIPT  ||
                    ...
                    if (isReliable
                        ...
                        and RELIABLE_SEQUENCED_WITH_ACK_RECEIPT is not useful unless you also know if the message was discarded.

                        The problem is that internally, message numbers are only assigned to reliable messages,
                        because message numbers are only used to discard duplicate message receipt and only reliable messages get
                        sent more than once. However, without message numbers getting assigned and transmitted, there is no way to
                        tell the sender about which messages were discarded. In fact, in looking this over I realized that
                        UNRELIABLE_SEQUENCED_WITH_ACK_RECEIPT introduced a bug, because the remote system assumes all message numbers
                        are used (no holes). With that send type, on packetloss, a permanent hole would have been created which eventually
                        would cause the system to discard all further packets.

                        So I have two options.
                        Either do not support ack receipts when sending sequenced, or write complex and major new systems.
                        UNRELIABLE_SEQUENCED_WITH_ACK_RECEIPT would need to send the message ID number on a special
                        channel which allowsfor non-delivery. And both of them would need to have a special range list
                        to indicate which message numbers were not delivered, so when acks are sent that can be indicated as well.
                        A further problem is that the ack itself can be lost - it is possible that the message can arrive but
                        be discarded, yet the ack is lost. On resend, the resent message would be ignored as duplicate, and
                        you'd never get the discard message either (unless I made a special buffer for that case too).*/
...
//                        internalPacket->reliability == UNRELIABLE_SEQUENCED_WITH_ACK_RECEIPT
                            )
                    {
                        ...
                        unreliableWithAckReceiptHistory.Push(UnreliableWithAckReceiptNode(
                                ...
                                congestionManager.GetRTOForRetransmission(internalPacket->timesSent + 1) + time), _FILE_AND_LINE_);
        ...
        for (unsigned int datagramIndex = 0; datagramIndex < packetsToSendThisUpdateDatagramBoundaries.Size(); datagramIndex++)
        {
            ...
            {
                auto &packet = packetsToSendThisUpdate[msgIndex];
                // If reliable or needs receipt
                if (packet->reliability != UNRELIABLE && packet->reliability != UNRELIABLE_SEQUENCED
                        )
                {
                    if (messageNumberNode == 0)
                        messageNumberNode = AddFirstToDatagramHistory(dhf.datagramNumber, packet->reliableMessageNumber, time);
                    else
                        messageNumberNode = AddSubsequentToDatagramHistory(messageNumberNode, packet->reliableMessageNumber);
                }

                RakAssert(updateBitStream.GetNumberOfBytesUsed() <= MAXIMUM_MTU_SIZE - UDP_HEADER_SIZE);
                WriteToBitStreamFromInternalPacket(&updateBitStream, packet, time);
                RakAssert(updateBitStream.GetNumberOfBytesUsed() <= MAXIMUM_MTU_SIZE - UDP_HEADER_SIZE);
                msgIndex++;
            }
            ...
            if (messageNumberNode == 0)
                AddFirstToDatagramHistory(dhf.datagramNumber, time);  // Unreliable, add dummy node
                                     ...
                                     RakNetRandom *rnr, CCTimeType currentTime)
{
    ...
    if (useSecurity)
    {
        unsigned char *buffer = bitStream->GetData();

        cat::u32 buffer_size = bitStream->GetNumberOfBitsAllocated() / 8;
...
bool ReliabilityLayer::IsSendThrottled(int MTUSize)
{
    ...
    for (unsigned i = 0; i < resendList.Size(); i++)
    {
        if (resendList[i])
...
//-------------------------------------------------------------------------------------------------------
unsigned
ReliabilityLayer::RemovePacketFromResendListAndDeleteOlderReliableSequenced(const MessageNumberType messageNumber,
                                                                            CCTimeType time,
                                                                            DataStructures::List<PluginInterface2 *> &messageHandlerList,
...
//    OrderingIndexType orderingIndex; // The ID used as identification for ordering channels
    ...
    InternalPacket *internalPacket = resendBuffer[messageNumber & RESEND_BUFFER_ARRAY_MASK];
                                                               ...
                                                               CCTimeType curTime)
{
    ...
    bitStream->AlignWriteToByteBoundary();
    RakAssert(internalPacket->dataBitLength < 65535);
    unsigned short s = (unsigned short) internalPacket->dataBitLength;
        ...
        internalPacket->reliability == RELIABLE_ORDERED_WITH_ACK_RECEIPT)
    {
        ...
        // Used for UNRELIABLE_SEQUENCED, RELIABLE_SEQUENCED, RELIABLE_ORDERED. 5 bits needed, write one byte
    ...
    bitStream->WriteAlignedBytes(internalPacket->data, BITS_TO_BYTES(internalPacket->dataBitLength));
...
InternalPacket *ReliabilityLayer::CreateInternalPacketFromBitStream(RakNet::BitStream *bitStream, CCTimeType time)
{
    ...
    unsigned char tempChar;
    bitStream->ReadBits(&(tempChar), 3);
    internalPacket->reliability = (const PacketReliability) tempChar;
    bool hasSplitPacket = false;
    bool readSuccess = bitStream->Read(hasSplitPacket); // Read 1 bit to indicate if splitPacketCount>0
...
//         internalPacket->reliability == RELIABLE_ORDERED_WITH_ACK_RECEIPT
    ...
    if (internalPacket->reliability == UNRELIABLE_SEQUENCED ||
        ...
        internalPacket->reliability == RELIABLE_ORDERED_WITH_ACK_RECEIPT)
    {
        // Used for UNRELIABLE_SEQUENCED, RELIABLE_SEQUENCED, RELIABLE_ORDERED. 4 bytes.
        bitStream->Read(internalPacket->orderingIndex);
        // Used for UNRELIABLE_SEQUENCED, RELIABLE_SEQUENCED, RELIABLE_ORDERED. 5 bits needed, Read one byte
    ...
    if (!readSuccess || internalPacket->dataBitLength == 0 || internalPacket->reliability >= NUMBER_OF_RELIABILITIES ||
    ...
    bool bitStreamSucceeded = bitStream->ReadAlignedBytes((unsigned char *) internalPacket->data, BITS_TO_BYTES(internalPacket->dataBitLength));
...
void ReliabilityLayer::SplitPacket(InternalPacket *internalPacket)
{
    ...
    int maximumSendBlockBytes = GetMaxDatagramSizeExcludingMessageHeaderBytes() - BITS_TO_BYTES(GetMaxMessageHeaderLengthBits());
    ...
    do
    {
        int byteOffset = splitPacketIndex * maximumSendBlockBytes;
    ...
    for (int i = 0; i < (int) internalPacket->splitPacketCount; i++)
    {
        ...
        RakAssert(internalPacketArray[i]->dataBitLength < BYTES_TO_BITS(MAXIMUM_MTU_SIZE));
        RakAssert(!internalPacketArray[i]->messageNumberAssigned);
...
void ReliabilityLayer::InsertIntoSplitPacketList(InternalPacket *internalPacket, CCTimeType time)
{
    ...
    unsigned index = splitPacketChannelList.GetIndexFromKey(internalPacket->splitPacketId, &objectExists);
    if (!objectExists)
    {
    ...
    auto &firstPacket = splitPacketChannelList[index]->firstPacket;

    // Return download progress if we have the first packet, the list is not complete, and there are enough packets to justify it
    if (splitMessageProgressInterval && firstPacket &&
        splitPacketChannelList[index]->splitPacketList.Size() != firstPacket->splitPacketCount &&
        (splitPacketChannelList[index]->splitPacketList.Size() % splitMessageProgressInterval) == 0)
    {
        // Return ID_DOWNLOAD_PROGRESS
        // Write splitPacketIndex (SplitPacketIndexType)
        ...
        unsigned int length = sizeof(MessageID) + sizeof(unsigned int) * 2 + sizeof(unsigned int) +
                              (unsigned int) BITS_TO_BYTES(firstPacket->dataBitLength);
        ...
        temp = (unsigned int) internalPacket->splitPacketCount;
        memcpy(progressIndicator->data + sizeof(MessageID) + sizeof(unsigned int) * 1, &temp, sizeof(unsigned int));
        temp = (unsigned int) BITS_TO_BYTES(firstPacket->dataBitLength);
        memcpy(progressIndicator->data + sizeof(MessageID) + sizeof(unsigned int) * 2, &temp, sizeof(unsigned int));

        memcpy(progressIndicator->data + sizeof(MessageID) + sizeof(unsigned int) * 3, firstPacket->data,
               (size_t) BITS_TO_BYTES(firstPacket->dataBitLength));
...
//-------------------------------------------------------------------------------------------------------
InternalPacket *
ReliabilityLayer::BuildPacketFromSplitPacketList(SplitPacketChannel *splitPacketChannel, CCTimeType time)
{
#if PREALLOCATE_LARGE_MESSAGES == 1
...
#else
    // Reconstruct
    InternalPacket *internalPacket = CreateInternalPacketCopy(splitPacketChannel->splitPacketList[0], 0, 0, time);
    internalPacket->dataBitLength = 0;
    for (unsigned j = 0; j < splitPacketChannel->splitPacketList.Size(); j++)
    ...
    for (unsigned j = 0; j < splitPacketChannel->splitPacketList.Size(); j++)
    {
        InternalPacket *splitPacket = splitPacketChannel->splitPacketList[j];
        size_t splitPacketPartLength = BITS_TO_BYTES(splitPacketChannel->splitPacketList[j]->dataBitLength);
        memcpy(internalPacket->data + BITS_TO_BYTES(offset), splitPacket->data, splitPacketPartLength);
                                                                 ...
                                                                 BitStream &updateBitStream)
{
    ...
    unsigned int i = splitPacketChannelList.GetIndexFromKey(splitPacketId, &objectExists);
    SplitPacketChannel *splitPacketChannel = splitPacketChannelList[i];
...
//-------------------------------------------------------------------------------------------------------
InternalPacket *
ReliabilityLayer::CreateInternalPacketCopy(InternalPacket *original, int dataByteOffset, size_t dataByteLength,
                                           CCTimeType time)
{
...
RakNetStatistics *ReliabilityLayer::GetStatistics(RakNetStatistics *rns)
{
    ...
    for (unsigned i = 0; i < RNS_PER_SECOND_METRICS_COUNT; i++)
    {
...
void ReliabilityLayer::PushPacket(CCTimeType time, InternalPacket *internalPacket, bool isReliable)
{
    BitSize_t bitsForThisPacket = BYTES_TO_BITS(
            BITS_TO_BYTES(internalPacket->dataBitLength) + BITS_TO_BYTES(internalPacket->headerLength));
...
ReliabilityLayer::AddFirstToDatagramHistory(DatagramSequenceNumberType datagramNumber,
...
//-------------------------------------------------------------------------------------------------------
ReliabilityLayer::MessageNumberNode *
ReliabilityLayer::AddSubsequentToDatagramHistory(MessageNumberNode *messageNumberNode,
                                                 DatagramSequenceNumberType messageNumber)
{
...
//-------------------------------------------------------------------------------------------------------
void
ReliabilityLayer::AllocInternalPacketData(InternalPacket *internalPacket, InternalPacketRefCountedData **refCounter,
