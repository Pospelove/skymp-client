	...
	if(s_runtimeDirectory.empty())
	{
		...
		if(runtimePathLength && (runtimePathLength < sizeof(runtimePathBuf)))
		{
			...
			if(lastSlash != std::string::npos)	// if we don't find a slash something is VERY WRONG
			{
				...
				gLog << "runtime root = " << s_runtimeDirectory << std::endl;
			}
			else
			{
				gLog << "no slash in runtime path? (" << runtimePath << ")" << std::endl;
			}
		}
		else
		{
			char buf[1024];
			sprintf_s<sizeof buf>(buf, "couldn't find runtime path (len = %d, err = %08X)", runtimePathLength, GetLastError());
			gLog << buf << std::endl;
		}
	}
