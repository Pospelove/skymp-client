...
static const std::vector<const char *> gPacketNames = {
	// client ----> server
	"ID_HANDSHAKE",
	"ID_UPDATE_MOVEMENT",
	"ID_UPDATE_WORLDSPACE",
...
inline RakNet::MessageID GetPacketID(const std::string &name)
{
	for (uint8_t i = 0; i != (uint8_t)~0; ++i)
	{
		RakNet::MessageID id = i + ID_USER_PACKET_ENUM;
		if (GetPacketName(id) == name)
			return id;
	}
	return 0;
}