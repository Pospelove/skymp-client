...
public:
	...
	virtual void Output(std::wstring message, bool isNotification = false) = 0;
	virtual void SendError(std::string message) = 0;
	virtual void MoveTo(NiPoint3 pos, float angle, uint32_t locationID) = 0;
	virtual void SetPlayerMovement(uint16_t playerID, ci::MovementData movement) = 0;
	virtual void SetPlayerName(uint16_t playerID, std::wstring name) = 0;
	virtual void CreatePlayer(uint16_t playerID, std::wstring name, ci::LookData look, NiPoint3 pos) = 0;
	virtual void DestroyPlayer(uint16_t playerID) = 0;
	virtual void SetPlayerPause(uint16_t playerID, bool pause) = 0;
	virtual void SetPlayerLook(uint16_t playerID, ci::LookData look) = 0;
	virtual void SetPlayerFurniture(uint16_t playerID, uint32_t objectID) = 0;
	virtual void ShowRaceMenu() = 0;
	virtual void HideDialog() = 0;
	virtual void ShowDialog(std::wstring title, ci::Dialog::Style style, std::wstring text, int32_t defaultIndex, std::function<void(ci::Dialog::Result)> sendResult) = 0;
	virtual void CreateObject(uint32_t objectID, bool isNative, uint32_t baseFormID, uint32_t locationID, NiPoint3 pos, NiPoint3 rot) = 0;
	virtual void DestroyObject(uint32_t objectID) = 0;
	virtual void SetObjectState(uint32_t objectID, NiPoint3 pos, NiPoint3 rot, uint32_t locationID) = 0;
	virtual void SetObjectName(uint32_t objectID, std::wstring name) = 0;
	virtual void SetObjectBehavior(uint32_t objectID, int32_t type, bool isOpen, uint32_t teleportTargetID, bool isDisabled, uint32_t itemsCount, uint32_t itemTypeID, uint16_t hostPlayerID, std::vector<std::pair<uint32_t, uint32_t>> toAdd, std::vector<std::pair<uint32_t, uint32_t>> toRemove) = 0;
	virtual void SetObjectMotionType(uint32_t objectID, int32_t motionTypeID) = 0;
	virtual void AddToPlayerInventory(uint16_t playerID, std::vector<std::pair<uint32_t, uint32_t>> pairsOfTypeAndCount, bool add) = 0;
	virtual void SetPlayerSpellList(uint16_t playerID, std::vector<uint32_t> spellList) = 0;
	virtual void PlayAnimation(uint16_t playerID, uint8_t animID) = 0;
	virtual void SetPlayerAVs(uint16_t playerID, std::vector<std::pair<uint8_t, ci::AVData>> avs) = 0;
	virtual void SetPlayerEquipment(uint16_t playerID, uint32_t rightHand, uint32_t leftHand, std::vector<uint32_t> armor, uint32_t ammo) = 0;
	virtual void SetPlayerMagicEquipment(uint16_t playerID, uint32_t rightHandSpell, uint32_t leftHandSpell) = 0;
	virtual void FireBow(uint16_t playerID, uint32_t power) = 0;
	virtual void FireMagic(uint16_t playerID, int32_t handID) = 0;
	virtual void SetItemTypeData(uint32_t itemTypeID, ItemTypeData itemTypeData) = 0;
	virtual void SetWeather(uint32_t weatherTypeOrID) = 0;
	virtual void SetGlobal(uint32_t globalID, float value) = 0;
	virtual void SetSilentInventoryChanges(bool silent) = 0;
	virtual void ExecuteCommand(std::string cmdText, int32_t cmdEngine) = 0;
	virtual void SetPlayerChatBubble(uint16_t playerID, std::wstring text, uint32_t ms) = 0;
	virtual void SetSpellData(uint32_t spellID, uint32_t formID, std::vector<EffectItemData> effects) = 0;
	virtual void SetEffectData(uint32_t effectID, uint32_t formID, uint8_t archetype, uint8_t castingType, uint8_t delivery) = 0;
	virtual void InitDataSearch() = 0;
	virtual void CreateText3D(uint32_t text3dID, NiPoint3 pos, std::wstring text) = 0;
